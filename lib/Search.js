...
class Search extends Request {
   ...
   */
  minScore(score) {
    this._query.set("min_score", score)

    return this
  }

  /**
   * Sets the MAXIMUM score of requested images
   *
   * @param {number} score
   *
   * @return {Search}
   */
  maxScore(score) {
    this._query.set("max_score", score)
  ...
  async exec() {
    ...
    if (min != null && isNaN(min)) {
      throw new TypeError("Given amount of minimal score must be a number.")
    }
